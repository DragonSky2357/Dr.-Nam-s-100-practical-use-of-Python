...
KAKAO_TOKEN = "RCa4usBySG61Z8LdIqhsXlbWk9H2q4j15nBqDAo9dNoAAAFxRmAzbw"
...
def get_hotdeal(keyword):
    url = "https://slickdeals.net/newsearch.php?src=SearchBarV2&q={}&pp=10".format(
        keyword)
    ...
    for r in rows:
        ...
        price = r.select("span.price")[0].text.replace(
            "$", "").replace("from", "").strip()
        if price.find("/") >= 0 or price == "" or price == "Free":
            continue
        price = float(price)
