...
def naver_translate(word):
    try:
        url = "https://ac.dict.naver.com/enendict/ac?q={}&q_enc=utf-8&st=11001".format(
            word)
        r = requests.get(url)
        j = json.loads(r.text)
...
def quize():
    ...
    for q in quize_list:
        ...
        for j in range(chance):
            user_input = str(input("단어 입력 => ")).strip().lower()
            ...
            if user_input == english:
                ...
                n = chance-(j+1)
                if j == 0:
                    print("{} 가 압닙니다. {} 번 기화가 남아있습니다.".format(user_input, n))
                elif j == 1:
                    print("{} 가 압닙니다. {} 번 기화가 남아있습니다. 힌트: {}로 시작".format(
                    ...
                    print("오답입니다. 정답은 {} 입니다.".format(english))
                    os.system("pause")
